...
- 게시·입금 일정 캘린더 보기를 추가한다.
- 입금 대기 및 확인 필요 거래 필터 보기를 구성한다.
- 홈 대시보드용 연결된 보기와 핵심 지표를 배치한다.
- 기본 표에서 상세 속성을 숨기고 핵심 열만 노출한다.
...
- OAuth 템플릿 복제 응답의 `duplicated_template_id`를 사용자별 Duepick 홈으로 저장하고 그 아래 거래 데이터 소스를 자동 탐색한다.
- 기준 Duepick 홈 템플릿은 전체 너비, 상태별 `진행 현황` 보드, `초안 일정` 캘린더 보기를 제공한다.
- `Duepick 공식 템플릿`은 위 후속 계획과 빈 데이터 상태 검수가 끝날 때까지 비공개로 유지한다.
